...
GPIO_PinInit(GPIOE, 0U, &led_config);
delay_seconds(1);
GPIO_WritePinOutput(GPIOE, 0U, true);
    ...
    // Add a small delay to ensure MC33664 is ready after initialization
    for (volatile int i = 0; i < 50000; i++); // Delay ~5 ms
}
 ...
 */
#define BUFFER_SIZE 5 // 40-bit frame = 5 bytes
...
static bool SlaveIF_writeRegister(uint8_t regAddress, uint16_t data) {
    ...
    // Step 1: Construct 40-bit frame (Table 24 in MC33771A datasheet)
    PRINTF("Step 1: Constructing frame...\n\r\r");
 ...
 *          activity on the TPL line, which wakes up the MC33771A from Sleep Mode.
 ...
 *       The MC33771A will wake up upon detecting any activity on the TPL line.
 */
