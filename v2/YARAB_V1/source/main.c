...
int main(void)
{
	...
	gpio_pin_config_t cs = {kGPIO_DigitalOutput, 1}; // Configure as output, initial state low

	// Initialize GPIO pins used for status LEDs
	GPIO_PinInit(GPIOB, 19U, &led_config);  // Example: Green LED
	//GPIO_PinInit(GPIOC, 11U, &led_config);
	//GPIO_PinInit(GPIOA, 12U, &initB_config);
	GPIO_PinInit(BOARD_LED_BLUE_GPIO, BOARD_LED_BLUE_GPIO_PIN, &led_config);
	GPIO_PinInit(GPIOC, 4U, &cs);
	//GPIO_WritePinOutput(GPIOC, 4U, 1);
